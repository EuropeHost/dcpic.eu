<div
    class="stat-card mx-auto mt-12 w-full max-w-6xl bg-white border shadow-lg rounded-xl p-8 transform transition duration-300"
>
    ...
    @if ($topLinkUsers->isNotEmpty())
        <ul class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
            @foreach ($topLinkUsers as $index => $user)
                <li class="user-item flex items-center space-x-3 bg-gray-50 p-2 rounded-lg transition
